...
constexpr Node_kind not_tree     = make_tree_node(121);
constexpr Node_kind and_tree     = make_tree_node(122);
constexpr Node_kind or_tree     = make_tree_node(123);
constexpr Node_kind lsh_tree     = make_tree_node(124);
...
struct And_tree : Tree {
  And_tree(const Token* k, Tree* t1,Tree *t2)
    : Tree(and_tree, k->loc), first(t1),second(t2) { }

  Tree* firstarg() const { return first; }
  Tree* secondarg() const { return second; }
...
struct Or_tree : Tree {
  Or_tree(const Token* k, Tree* t1,Tree *t2)
    : Tree(or_tree, k->loc), first(t1),second(t2) { }
...
struct Lsh_tree : Tree {
  Lsh_tree(const Token* k, Tree* t1,Tree *t2)
    : Tree(lsh_tree, k->loc), first(t1),second(t2) { }
