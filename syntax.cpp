...
pp_not(std::ostream& os, Not_tree* t) {
  os << "NOT " << group(pretty(t->t1));
}

void
pp_and(std::ostream& os, And_tree* t) {
  os << "AND { " << group(pretty(t->first))
  	 << ", "
...
void
pp_lsh(std::ostream& os, Lsh_tree* t) {
  os << "Lsh { " << group(pretty(t->first))
  	 << ", "
  	 << group(pretty(t->second))
  	 << "}";
}
...
print(std::ostream& os, Tree* t) {
  ...
  case lsh_tree:  return pp_lsh(os, as<Lsh_tree>(t));
