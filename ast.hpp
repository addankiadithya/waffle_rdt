...
constexpr Node_kind t_term      = make_term_node(13);
constexpr Node_kind f_term      = make_term_node(14);
constexpr Node_kind not_term      = make_term_node(15);
constexpr Node_kind and_term      = make_term_node(16);
constexpr Node_kind or_term      = make_term_node(17);
constexpr Node_kind lsh_term      = make_term_node(18);
// Numeric terms
...
// Represents the constant term 'T'.
struct T : Term {
  T(Type* t) 
    : Term(t_term, t) { }
  T(const Location& l, Type* t) 
    : Term(t_term, l, t) { }
};

// Represents the constant term 'F'.
struct F : Term {
  F(Type* t) 
    : Term(f_term, t) { }
  F(const Location& l, Type* t) 
    : Term(f_term, l, t) { }
};
...
// Represents the term 'Not t'.
struct Not : Term {
  Not(Type* t0, Term* t) 
    : Term(not_term, t0), t1(t) { }
  Not(const Location& l, Type* t0, Term* t) 
    : Term(not_term, l, t0), t1(t) { }
...
// Represents the term 'And t t'.
struct And : Term {
  And(Type* t0, Term* t1,Term* t2) 
    : Term(and_term, t0), first(t1), second(t2) { }
  And(const Location& l, Type* t0, Term* t1,Term* t2) 
    : Term(and_term, l, t0),  first(t1), second(t2)  { }

  Term* firstarg() const { return first; }
  Term* secondarg() const { return second; }

  Term* first;
  Term* second;
};

// Represents the term 'Or t t'.
struct Or : Term {
  Or(Type* t0, Term* t1,Term* t2) 
    : Term(or_term, t0), first(t1), second(t2) { }
  Or(const Location& l, Type* t0, Term* t1,Term* t2) 
    : Term(or_term, l, t0),  first(t1), second(t2)  { }
...
// Represents the term 'Lsh t t'.
struct Lsh : Term {
  Lsh(Type* t0, Term* t1,Term* t2) 
    : Term(lsh_term, t0), first(t1), second(t2) { }
  Lsh(const Location& l, Type* t0, Term* t1,Term* t2) 
    : Term(lsh_term, l, t0),  first(t1), second(t2)  { }
